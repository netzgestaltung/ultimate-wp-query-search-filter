...
if (!class_exists('uwpqsfprocess')) {
  class uwpqsfprocess {
    function __construct() {
      ...
      //front ajax
      add_action('wp_ajax_nopriv_uwpqsf_ajax', array(
        $this,
        'uwpqsf_ajax'
      ));
      add_action('wp_ajax_uwpqsf_ajax', array(
        $this,
        'uwpqsf_ajax'
      ));
    ...
    function uwpqsf_search_query($query) {
      if ($query->is_search()) {
        if ($query->query_vars['s'] == 'uwpsfsearchtrg' && isset($_GET['uformid'])) {
          ...
          $ordertype = apply_filters('uwpqsf_dmeta_type', $ordertype, $getdata, $id);
    ...
    function uwpqsf_ajax() {
      ...
      if (isset($formid) && wp_verify_nonce($nonce, 'uwpsfsearch')) {
        ...
        $order = !empty($options[0]['sorder']) ? $options[0]['sorder'] : null;
        $ordertype = !empty($options[0]['otype']) ? $options[0]['otype'] : null;
        $pagenumber = isset($_POST['pagenum']) ? intval(filter_var($_POST['pagenum'], FILTER_SANITIZE_NUMBER_INT)) : null;
        $options = get_post_meta($id, 'uwpqsf-option', true);
        $cpts = get_post_meta($id, 'uwpqsf-cpt', true);
        ...
        if ( isset($getdata['order']) && !empty($getdata['order']) ) {
          $order = $getdata['order'];
        }
        ...
        $result = apply_filters('uwpqsf_result_tempt', $results, $arg, $id, $getdata, $pagenumber);

        echo $result;

      }
      else {
        echo 'There is error here';
      }
      die;
    ...
    function uajax_result($arg, $id, $pagenumber, $getdata) {
      ...
      if ($query->have_posts()) {
        ...
        while ($query->have_posts()) {
          $query->the_post();
          $post_id = get_the_ID();
          $download_description = apply_filters('the_content', get_post_meta($post_id, '_download-description', true));
          if (!empty($download_description)) {
            $excerpt = $download_description;
          }
          else {
          ...
          $html .= '<article id="post-' . $post_id . '" ' . implode(' ', $post_class) . '>';
          $html .= '<div class="inner clearfix">';
          $html .= '<h2 class="title"><a href="' . get_permalink() . '" rel="bookmark">' . get_the_title() . '</a></h2>';
          $html .= '<div class="content">' . $excerpt . '</div></article>';
        }
    ...
    function ajax_pagination($pagenumber, $pages = '', $range = 4, $id, $getdata) {
      ...
      if (1 != $pages) {
        ...
        if ($paged > 2 && $paged > $range + 1 && $showitems < $pages) $html .= '<a id="1" class="upagievent" href="#">&laquo; ' . __("First", "UWPQSF") . '</a>';
        $previous = $paged - 1;
    ...
    function ufront_js() {
      ...
      if (isset($themenames)) {
        foreach ($themeops as $k) {
          ...
          wp_enqueue_script('uwpqsfscript', plugins_url('/scripts/uwpqsfscript.js', __FILE__), array(
            'jquery'
          ), '1.0', true);
